...
function Table ({docType}) {
  ...
  const myActions = actions.view.Pages.LettersOfGuarantee.Table
  const myState = state.view.Pages.LettersOfGuarantee
  const collection = myState.Table || [];
  const il = infiniteLoader('LettersOfGuarantee', myActions.loadDocumentKeys);
  return (
    <AutoSizer>
      {({ height, width }) => (
        <RTable
        ...
        >
          ...
          />
          <Column label='Buyers' dataKey='buyers' width={400} />
          <Column label='Sellers' dataKey='sellers' width={300} />
