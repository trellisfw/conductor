...
function MyHeader (props) {
  ...
  let validState = _.get(myState, ['document', '_meta', 'services', 'fl-sync', 'valid', 'status']);
  let valid = validState === true ?
  ...
  let signed = Object.keys(_.get(myState, ['document', 'signatures']) || {}).length > 0 
    ? <div css={{ display: 'flex', alignItems: 'center' }}>
        <SignedIcon />
        <div css={{ marginLeft: 3, color: '#02A12B', marginRight: 7 }}>
          {'Signed'}
  ...
  let unmask = _.get(myState, ['document', 'unmask'])
    ? 'MASKED' 
    : null;
