  ...
  Modals: {
    ...
    TPSelectModal: {
      ...
      checkPending({ state, actions }, props) {
        state.view.Modals.TPSelectModal.pending = !state.view.Modals
          .TPSelectModal.pending;
      },
    },
    RulesModal: {
      ...
      async viewMappings({ state, actions }) {
        ...
        if (inverse) {
          ...
          Object.values(tps).forEach((tp) => {
            Object.keys(tp[listType] || {}).forEach((masterid) => {
              ...
              let entry =
                tp.name +
                (tpCityAndState ? " - " + tp.city + ", " + tp.state : "");
          ...
          Object.keys(list)
            ...
            .forEach((key) => {
              let cityAndState =
                list[key].city && list[key].state ? true : false;
              obj[key] = {
                name:
                  list[key].name +
                  (cityAndState
                    ? " - " + (list[key].city + ", " + list[key].state)
                    : ""),
                partners: Object.keys(list[key]["trading-partners"] || {}).map(
                  (masterid) => {
        ...
        obj = _.orderBy(obj, (item) =>
          item.name ? item.name.toLowerCase() : item.name
        );
      ...
      async searchMappings({ state, actions }, value) {
        ...
        state.view.Modals.RulesModal.Edit.mappingSearchResults = results.map(
          (item) => item.refIndex
        );
      },
      ...
      textChanged({ state, actions }, result) {
        ...
        state.view.Modals.RulesModal.Edit.rule[result.key].values = _.keyBy(
          values,
          "key"
        );
      },
    ...
    FileDetailsModal: {
      ...
      showDocument({ state }, { resourceId }) {
        ...
        DOC_TYPES.forEach((docType) => {
          let docKey = _.chain(state.oadaHelper.data[docType])
            .findKey({ _id: resourceId })
            .value();
          if (docKey) {
      ...
      viewPDF({ state, actions }, { docKey, docType }) {
        ...
        if (docType === "documents" || docType === "application/pdf") {
          ...
          let shared = state.oada.data[docType][docKey].shared
            ? "shared"
            : "bookmarks";
          let path = tp
            ? `${state.oadaHelper.path}/${shared}/trellisfw`
            : `/bookmarks/trellisfw`;
      ...
      share({ state, actions }) {
        ...
        const shareKeys = _.map(
          state.view.Modals.FileDetailsModal.share,
          (share, key) => {
            return key;
          }
        );
  ...
  Pages: {
    Reports: {
      ...
      async saveReports({ state, actions }) {
        ...
        if (
          !Object.keys(state.oada.data.Reports[selectedReport]).some((key) => {
      ...
      eventLog: {
        Table: {
          ...
          toggleCheckbox({ state, actions }, date) {
            state.oada.data.Reports.eventLog[date].checked = !state.oada.data
              .Reports.eventLog[date].checked;
          },
          ...
          async saveReports({ state, actions }) {
            ...
            await Promise.each(
              Object.keys(myState)
                .filter(
                  (date) =>
                    myState[date] !== null && myState[date] !== undefined
                )
      ...
      userAccess: {
        Table: {
          ...
          toggleCheckbox({ state, actions }, date) {
            state.oada.data.Reports.userAccess[date].checked = !state.oada.data
              .Reports.userAccess[date].checked;
          },
      ...
      documentShares: {
        Table: {
          ...
          toggleCheckbox({ state, actions }, date) {
            state.oada.data.Reports.documentShares[date].checked = !state.oada
              .data.Reports.documentShares[date].checked;
          },
        ...
        async onRowClick({ state, actions }, props) {
          ...
          switch (rowData.type) {
            ...
            default:
              ...
              state.view.MessageLog.path = `oada.data.documents.${docKey}`;
              state.view.Modals.FileDetailsModal.docType =
                doc.docType || rowData.docType;
              state.view.Modals.FileDetailsModal.docKey =
                doc.docKey || rowData.docKey;
              state.view.Modals.FileDetailsModal.open = true;
              state.view.Modals.FileDetailsModal.sharedWith = [];
              state.view.Modals.FileDetailsModal.sharedWith = await actions.oadaHelper.getTradingPartners(
                { docType, docKey }
